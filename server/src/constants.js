module.exports = {
  ACCESS_TOKEN_TIME: parseInt(process.env.ACCESS_TOKEN_TIME, 10) || 60 * 60,
  SALT_ROUNDS: parseInt(process.env.SALT_ROUNDS, 10) || 5,
  JWT_SECRET: process.env.JWT_SECRET || 'asdasdasd4as5d4as8d7a8sd4as65d4a8sd7asd4as56d4',
  SQUADHELP_BANK_NUMBER: process.env.SQUADHELP_BANK_NUMBER || '4564654564564564',
  SQUADHELP_BANK_NAME: process.env.SQUADHELP_BANK_NAME || 'SquadHelp',
  SQUADHELP_BANK_CVC: process.env.SQUADHELP_BANK_CVC || '453',
  SQUADHELP_BANK_EXPIRY: process.env.SQUADHELP_BANK_EXPIRY || '11/26',
  CUSTOMER: 'customer',
  CREATOR: 'creator',
  CREATOR_ENTRIES: 'creator_entries',
  CONTEST_STATUS_ACTIVE: 'active',
  CONTEST_STATUS_FINISHED: 'finished',
  CONTEST_STATUS_PENDING: 'pending',
  CONTESTS_DEFAULT_DIR: 'public/contestFiles/',
  STATIC_IMAGES_PATH: 'public/static/',
  NAME_CONTEST: 'name',
  LOGO_CONTEST: 'logo',
  TAGLINE_CONTEST: 'tagline',
  OFFER_STATUS_PENDING: 'pending',
  OFFER_STATUS_REJECTED: 'rejected',
  OFFER_STATUS_WON: 'won',
  OFFER_STATUS_REVIEW: 'review',
  OFFER_STATUS_APPROVED: 'approved',
  OFFER_STATUS_DECLINED: 'declined',
  FILES_PATH: 'public/',
  SOCKET_CONNECTION: 'connection',
  SOCKET_SUBSCRIBE: 'subscribe',
  SOCKET_UNSUBSCRIBE: 'unsubscribe',
  NOTIFICATION_ENTRY_CREATED: 'onEntryCreated',
  NOTIFICATION_CHANGE_MARK: 'changeMark',
  NOTIFICATION_CHANGE_OFFER_STATUS: 'changeOfferStatus',
  NEW_MESSAGE: 'newMessage',
  CHANGE_BLOCK_STATUS: 'CHANGE_BLOCK_STATUS',
  LOGS: {
    ERRORS_FILE_NAME: 'errors.log',
    SCHEDULE_TIME: {
      hour: 12,
      minute: 0,
    },
    DIR: "logs"
  }
};
